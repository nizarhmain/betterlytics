...
async fn run_inserter_worker(
    ...
    loop {
        ...
        tracing::debug!(worker_id = worker_id, site_id = %row.site_id, visitor_id = %row.visitor_id, session_id = %row.session_id, url = %row.url, timestamp = %row.timestamp, device_type = %row.device_type, "Prepared row for ClickHouse insertion");
