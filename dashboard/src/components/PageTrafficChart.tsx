...
export default function PageTrafficChart({ pageTrafficData, isLoading, granularity }: PageTrafficChartProps) {
  ...
  const { chartData, tooltipLabelFormatter, scale, ticks } =
    ...
    return (
      <div className='flex h-80 items-center justify-center'>
        <div className='flex flex-col items-center'>
          <div className='h-8 w-8 animate-spin rounded-full border-3 border-t-blue-600 border-r-transparent border-b-blue-600 border-l-transparent'></div>
        </div>
      </div>
    );
  if (pageTrafficData.length === 0)
    return <div className='flex h-80 items-center justify-center text-gray-500'>No data available</div>;
      ...
      <ResponsiveContainer width='100%' height='100%'>
        <AreaChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
          ...
          <YAxis
            ...
            width={48}
          />
          <Tooltip labelFormatter={tooltipLabelFormatter} formatter={(value) => [value, 'Views']} />
          <Area
            type='monotone'
            dataKey='views'
            stroke='#3b82f6'
