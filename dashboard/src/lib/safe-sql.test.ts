...
describe('SafeSQL tests', () => {
  it('returns empty string and object on no input', () => {
    const expected = {
      taggedSql: '',
      taggedParams: {},
    };
  ...
  it('returns the raw SQL if no parameters are passed', () => {
    const expected = {
  ...
  it('returns the tagged SQL when parameters are sent', () => {
    const expected = {
  ...
  it('correctly returns recursively used safeSql', () => {
    const expected = {
      taggedSql:
        'SELECT uniq(visitor_id) FROM analytics.events WHERE site_id = {site_id:String} AND event_type = {event_type:String}',
      taggedParams: {
        site_id: 'test',
        event_type: 'custom',
      },
  ...
  it('returns correct values using list helper', () => {
    const expected = {
      taggedSql:
        'SELECT uniq(visitor_id) FROM analytics.events WHERE url = {page_0:String} OR url = {page_1:String} OR url = {page_2:String}',
      taggedParams: {
        page_0: 'a',
        page_1: 'b',
        page_2: 'c',
  ...
  it('returns the raw sql when SQL.Unsafe is used', () => {
    const expected = {
      taggedSql: "SELECT uniq(visitor_id) FROM analytics.events WHERE site_id = 'hello world'",
      taggedParams: {},
    };

    const actual = safeSql`SELECT uniq(visitor_id) FROM analytics.events WHERE site_id = ${SQL.Unsafe("'hello world'")}`;
