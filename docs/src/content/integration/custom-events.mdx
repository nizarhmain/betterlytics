...
- **What** was clicked? (`buttonText: "Get Started"`)
- **Where** on the page? (`location: "header"`)
...
function trackCheckoutStep(step, additionalData = {}) {
  betterlytics("checkout-step", {
    step: step,
    ...additionalData,
  });
}

// Usage: trackCheckoutStep('shipping-info', { method: 'express' });
```
