...
export default async function DashboardPage({ params, searchParams }: DashboardPageParams) {
  ...
  const { startDate, endDate, granularity, queryFilters } = await BAFilterSearchParams.decodeFromParams(searchParams);

  const analyticsCombinedPromise = fetchPageAnalyticsCombinedAction(dashboardId, startDate, endDate, 5, queryFilters);
