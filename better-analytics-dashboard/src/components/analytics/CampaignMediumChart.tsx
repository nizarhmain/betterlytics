...
interface CampaignMediumChartProps {
  data: CampaignMediumBreakdownItem[];
  isLoading: boolean;
}

export default function CampaignMediumChart({ data, isLoading }: CampaignMediumChartProps) {
  if (isLoading) {
    return (
      <div>
  ...
  const chartData = useMemo(() => {
    ...
    return data.map((item) => ({
      name: item.medium,
      value: item.visitors,
      color: getCampaignSourceColor(item.medium),
  ...
  if (chartData.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow p-6 min-h-[300px] flex items-center justify-center">
        <p className="text-gray-500">No medium breakdown data available for campaigns.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-lg font-bold text-gray-900 mb-1">Campaign Traffic by Medium</h2>
      <p className="text-sm text-gray-500 mb-4">Distribution of campaign visitors by medium</p>
